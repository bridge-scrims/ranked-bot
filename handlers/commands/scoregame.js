...
module.exports.run = async (interaction) => {
    if (interaction.options.getSubcommand() === 'arguments') {
        ...
        if (interaction.member.roles.cache.has(roles.scorer)) {
            if (interaction.channel.name.startsWith("game-")) {
                ...
                let wName = await gameFunctions.getName(winner);
                let lName = await gameFunctions.getName(loser);

                let wWins = await gameFunctions.getWins(winner);
                let wLosses = await gameFunctions.getLosses(winner);
                ...
                let lWins = await gameFunctions.getWins(loser);
                let lLosses = await gameFunctions.getLosses(loser);
                let lWs = await gameFunctions.getWinstreak(loser);
