    ...
    return new Promise(function (resolve, reject) {
        db.serialize(async function () {
            await db.get('SELECT * FROM guild WHERE id = ?', [id], async function (err, rows, fields) {
                if (err) reject(err);
                if (!rows) {
                    var stmt = db.prepare("INSERT INTO guild(id, voicerole) VALUES ($id, $role)");
                    stmt.run({ $id: id, $role: role });
                    stmt.finalize();
                    resolve("Updated!");
                } else {
                    let stmt = await db.prepare("UPDATE guild SET voicerole = @role WHERE id = @id");
        ...
        db.serialize(async function () {
            await db.get('SELECT * FROM guild WHERE id = ?', [id], function (err, rows, fields) {
                ...
                if (!rows) {
                    ...
                    if (rows.voicerole === null || !rows.voicerole || rows.voicerole.length < 1) {
                        resolve(null);
                    } else {
                        resolve(rows.voicerole);
                    }
                }
            });
        });
